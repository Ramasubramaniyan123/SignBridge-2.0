...
import { getGestureByLabel } from "@/lib/gesture-data";
    ...
    <div className="container py-6 md:py-10 pb-24 md:pb-10">
      <div className="max-w-2xl mx-auto">
        <div className="flex items-start justify-between mb-8">
          <div>
            <h1 className="font-display text-3xl md:text-4xl font-bold mb-2">Detection History</h1>
        ...
        {history.length === 0 ? (
          ...
          <div className="space-y-2">
            {history.map((record, i) => {
              const gesture = getGestureByLabel(record.label);
              return (
                <motion.div
                  key={record.id}
                    ...
                    <CardContent className="p-4 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-lg overflow-hidden bg-secondary/30 flex-shrink-0">
                          {gesture ? (
                            <img src={gesture.image} alt={record.label} className="w-full h-full object-cover" />
                          ) : (
